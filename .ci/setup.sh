...
#            brew link --overwrite gcc  # previous variant to deal with conflict link
    ...
    if [[ $TASK == "gpu" ]]; then
        ...
        cd /tmp/
        sudo wget -q https://github.com/Microsoft/LightGBM/releases/download/v2.0.12/AMD-APP-SDKInstaller-v3.0.130.136-GA-linux64.tar.bz2 || exit -1
        sudo tar -xjf AMD-APP-SDK*.tar.bz2 || exit -1
        ...
        echo 'libamdocl64.so' | sudo tee --append $OPENCL_VENDOR_PATH/amdocl64.icd
        cd $HOME_DIRECTORY
    fi
    if [[ $TRAVIS == "true" ]]; then
